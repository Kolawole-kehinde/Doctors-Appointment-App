...
const Doctors = () => {
  ...
  // Apply filter based on speciality, default to all doctors if no filter applied
  useEffect(() => {
    setFilterDoc(
      speciality
  ...
  // List of doctor specialties to reduce code redundancy
  const specialties = [
    'General Physician',
    'Gynecologist',
    'Dermatologist',
  ...
  return (
    <div className="wrapper px-4 lg:px-0 font-outfit text-secondary-300 mt-5">
      <h4 className="text-xl leading-[27px]">Browse through the doctors specialist.</h4>

      <div className="flex flex-col md:flex-row items-start gap-5 mt-10">
        <div className="flex flex-col gap-4 text-base text-secondary-300 text-center">
          {specialties.map((item) => (
            <p
              key={item}
              onClick={() => navigate(speciality === item ? '/doctors' : `/doctors/${item}`)}
              className={`w-full md:w-[200px] truncate pl-3 py-1.5 border border-gray-300 rounded transition-all cursor-pointer`}
            >
              {item}
            </p>
          ))}
        ...
        <div className="w-full grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6">
          {filterDoc.map(({ id, image, name, speciality }) => (
            ...
            >
              <img src={image} alt={`${name}'s profile`} className="bg-blue-50 w-full h-auto object-cover" />
